...
class PluginManager(type):
    ...
    def __init__(self, name, bases, dict):
        ...
        if not os.path.exists(os.path.join(sys.path[0], self.__PluginPath)):
            os.mkdir(self.__PluginPath)
            sys.path.append(self.__PluginPath)
            ...
            if os.path.isdir(os.path.join(pluginPath, item)):
                ...
                if item.endswith('.py') and item != '__init__.py':
                    ...
                    if moduleName not in sys.modules:
                        fileHandle, filePath, dect = find_module(
                            moduleName, [pluginPath])
                        try:
                            moduleObj = load_module(moduleName, fileHandle,
                                                    filePath, dect)
                        finally:
                            if fileHandle: fileHandle.close()
        PluginManager.__PluginPath = 'Plugins'
    ...
    def GetPluginObject(self, pluginName=None):
        if pluginName is None:
            ...
            result = self.__AllPlugins[
                pluginName] if pluginName in self.__AllPlugins else None
            return result
...
class Model_Component(metaclass=PluginManager):
    def Start(self):
...
class Model_MenuObj(metaclass=PluginManager):
    def Start(self):
        print('Please write the Start() function')
...
class Model_ToolBarObj(metaclass=PluginManager):
    def Start(self):
        print('Please write the Start() function')
...
class Model_ParamPanelObj(metaclass=PluginManager):
    def Start(self):
        print('Please write the Start() function')
...
__ALLMODEL__ = (Model_ParamPanelObj, Model_ToolBarObj, Model_MenuObj,
                Model_Component)